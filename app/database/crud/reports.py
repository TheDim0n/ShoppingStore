...
def get_customer_sum_report(
    ...
    ).select_from(models.Purchase).join(
        models.Customer,
        models.Customer.uuid == models.Purchase.customer_uuid
    )
    ...
    ).select_from(models.Customer).join(
        report, report.c.uuid == models.Customer.uuid,
        isouter=True
    ).all()
